    ...
    fetch(address + '/api/friends/friendRequest', {
        ...
        .then(data => {
            if (data.status === 'User not found') {
                ...
                notificationText.innerText = "User not found! Check spelling, otherwise it does not exist";
            }
        })
        .catch(error => {
            console.error(error);
...
function setupChatContainer(friend) {
    ...
    newItem.innerHTML = `<i class="fa-solid fa-chevron-left" style="margin: 0; color:black;"></i>
...
function getFriendRequest() {
    ...
    fetch(address + `/api/friends/retrieveFriendRequest`, {
        ...
        .then(response => response.json())
        .then(data => {
            document.getElementById("iconNotificationFriendRequest1").style.display = 'none';
            document.getElementById("iconNotificationFriendRequest2").style.display = 'none';
            if (data.length > 0) {
                document.getElementById("iconNotificationFriendRequest1").style.display = 'block';
                document.getElementById("iconNotificationFriendRequest2").style.display = 'block';
            }
...
function challengeMini(friendName) {
    ...
    if (pendingChallenge) {
        notification.style.display = "flex";
        notificationText.innerText = "Waiting for " + pendingChallengedName +
        ...
        notification.style.display = "flex";
        notificationText.innerText = "Waiting for " + friendName + "!";
...
chatBar.addEventListener('keydown', (event) => {
    if (event.key === "Enter") {
        socket.emit('friendChat', {
            friendUsername: currentFriendDiscussion,
            chat: chatBar.value,
            token: token
        });
...
socket.on('friendIsChallenging', (challengerName) => {
    ...
    if (typeof cordova !== 'undefined') {
        cordova.plugins.notification.local.on('accept-action', function () {
        ...
        cordova.plugins.notification.local.on('decline-action', function () {
...
socket.on('loadAllMessagePending', (data) => {
    ...
    console.log(data);
    document.getElementById("iconNotification").style.display = 'none';
    data = data.filter(message => document.getElementById("notificationMini" + message.from) !== null);
    if (data.length > 0)
        document.getElementById("iconNotification").style.display = 'block';
...
function appendMessage(message) {
    let newItem = document.createElement('div');
    chatContainer.style.display = "flex";
    newItem.innerHTML = '<div class="user-message">' + message + '</div>';
...
socket.on('notConnectedMessage', (challengedName) => {
    ...
    pendingChallenge = false;
    notification.style.display = "flex";
    notificationText.innerText = "Oh no! " + challengedName + " is not connected! Or he is already in game...";
...
socket.on('notFriendMessage', (challengedName) => {
    ...
    pendingChallenge = false;
    notification.style.display = "flex";
    notificationText.innerText = "Oh no! " + challengedName + " is not your friend!";
})
...
socket.on('challengeDeclined', (challengedName) => {
    ...
    document.getElementById("cancelChallengeMini").style.display = "none";
    document.getElementById("ok-btn").style.display = "block";
    notificationText.innerText = "Oh no! " + challengedName + " has declined your challenge!";
})
...
function showMiniFriendList(friendList) {
    ...
    for (let i = 0; i < friendList.length; i++) {
        ...
        let idProfile = "profileMini" + friendList[i];
        ...
        newItem.innerHTML = ` <div class="miniFriend" style="font-size: 2em;" >
                                ...
                                <div class="miniFriendMenu" id=${idMiniFriendMenu} style="display: none; justify-content: space-between;"">
                                    ...
                                    <div class="icon_mini" id="${idProfile}">
        ...
        document.getElementById(idProfile).addEventListener('click', function () {
        ...
        document.getElementById(idRemove).addEventListener('click', function () {
            ...
            removeFriend(friendList[i]).then(() => console.log("Friend removed"));
        });
...
function challengedMini(challengerName) {
    document.getElementById("iconNotificationFight1").style.display = 'block';
    document.getElementById("iconNotificationFight2").style.display = 'block';
...
function showFriendRequestMini(friendRequest) {
    ...
    for (let i = 0; i < friendRequest.length; i++) {
        ...
        document.getElementById(acceptId).addEventListener('click', function () {
            acceptFriendRequest(friendRequest[i]).then(() => console.log("Friend request accepted"));
            dropdown.removeChild(newItem);
        });
        document.getElementById(declineId).addEventListener('click', function () {
            declineFriendRequest(friendRequest[i]).then(() => console.log("Friend request declined"));
            dropdown.removeChild(newItem);
        });
    }
}
...
document.getElementById("miniNotification").addEventListener('click', goNotification);
...
function goNotification() {
    document.getElementById("iconNotificationFight1").style.display = 'none';
    document.getElementById("iconNotificationFight2").style.display = 'none';
...
let notificationText = document.getElementById("notificationTextMessage");
